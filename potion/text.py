...
class Text:
    ...
    def glyph_color(self, glyph: Glyph) -> Color | None:
    ...
    def glyph_opacity(self, glyph: Glyph) -> int | None:
    ...
    def draw(self, camera: Camera, position: Point) -> None:
        ...
        # Apply tint
        if self.color:
            Renderer.set_texture_color_mod(self._cache_texture, self.color)

        # Apply opacity
        if self.opacity != 255:
            Renderer.set_texture_alpha_mod(self._cache_texture, self.opacity)
        ...
        # Clear tint
        if self.color:
            Renderer.clear_texture_color_mod(self._cache_texture)

        # Clear tint and opacity
        if self.opacity != 255:
            Renderer.clear_texture_alpha_mod(self._cache_texture)
    ...
    def _create_cache_texture(self) -> None:
        ...
        self._cache_texture.set_blend_mode(BlendMode.ALPHA_COMPOSITE)
